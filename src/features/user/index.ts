...
//         state.tags.list[tag.name] ? state.tags.list[tag.name].count++ : (state.tags.list[tag.name] = tag);
//         addDBTags(tag);
//       }
//     },
...
//             state.tags.active = state.tags.active.filter((item) => item !== tag.name);
...
// export const { addNote, updateNote, deleteNote, addFilters, insertDB, updateSnackbar } = notesSlice.actions;
